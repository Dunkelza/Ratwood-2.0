	...
	if(name in unknown_names)
		...
		if(any_cum_effect || drip)
			...
			if(!show_detail)
				...
				if(external)
					. += span_aiprivradio("[m1] body is [!external.has_dried_up ? "covered in cum" : "covered in dried cum"]!")
				if(facial)
					. += span_aiprivradio("[m1] face is [!facial.has_dried_up ? "glazed with cum" : "plastered with dried cum"]!")
				if(creampie && !drip)
					. += span_aiprivradio("[m1] crotch is [!creampie.has_dried_up ? "a cummy mess" : "stained with dried cum"]!")
				if(drip)
